...
export default function HomePage() {
  ...
  // Hook de verificación de edad - llamado al inicio del componente
  const {
  ...
  useEffect(() => {
    const handleStorageChange = () => {
      // Cuando se verifica la edad, recargar los anuncios
      fetchAds({ ...filters, userId: user?.id });
    };

    window.addEventListener("storage", handleStorageChange);
    return () => window.removeEventListener("storage", handleStorageChange);
  }, [fetchAds, filters, user?.id]);
  ...
  // Category select con verificación de edad
  ...
  // Open detail - navigates to the ad page
  ...
  const handleDelete = useCallback(
    async (adId) => {
      if (
        window.confirm(
          "¿Estás seguro de que quieres eliminar esta publicación? Esta acción también borrará las fotos permanentemente.",
        )
      ) {
