...
function AdCard({ ad, onOpenDetail, onEdit, onDelete, onFeatureAd, onApprovePayment, onRejectPayment }) {
  ...
  const remainingText = ad.destacado_hasta ? getRemainingTimeText(ad.destacado_hasta) : null
  ...
  const handleReject = useCallback((e) => {
    e.stopPropagation()
    if (window.confirm('¿Rechazar la solicitud de pago de esta publicación?')) {
      onRejectPayment(ad.id)
    }
  }, [ad.id, onRejectPayment])

  return (
    <article className={`card ${ad.destacado ? 'card--featured' : ''}`} onClick={handleClick}>
      <div className="card__image-container">
        {ad.destacado && (
          <span className="card__badge card__badge--featured" title={remainingText ? `Destacado (${remainingText})` : 'Publicación Destacada'}>
            <i className="fas fa-star"></i> Destacado {isOwner && remainingText ? `(${remainingText})` : ''}
          </span>
        )}
        {!ad.destacado && isPendingPayment && (
          <span className="card__badge card__badge--pending" title="Pago en revisión por el administrador">
            <i className="fas fa-clock"></i> Pago Pendiente ({ad.pago_dias || 7}d)
          </span>
        )}
        <span className={`card__badge ${ad.destacado || isPendingPayment ? 'card__badge--secondary' : ''}`}>
